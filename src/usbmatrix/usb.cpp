...
int USBConnection::Init() {
	usb_init(); /* initialize the library */
	usb_find_busses(); /* find all busses */
	usb_find_devices(); /* find all connected devices */

	if(!(usbdev = open_dev())) {
		matrixlog.Error() << "USB ERROR: USB Matrix Not Found!" << std::endl;
		return 0;
	}

	if(usb_set_configuration(usbdev, 1) < 0) {
		usb_close(usbdev);
		matrixlog.Error() << "USB ERROR: Error while initialising communication with USB matrix (setting config 1 failed)" << std::endl;
		return 0;
	}

	if(usb_claim_interface(usbdev, 0) < 0) {
		usb_close(usbdev);
		matrixlog.Error() << "USB ERROR: Error while initialising communication with USB matrix (claiming interface 0 failed)" << std::endl;
		return 0;
	}
...
int USBConnection::Send(char *data, int length) {
	return usb_bulk_write(usbdev, EP_OUT, data, length, USB_TIMEOUT);
}


int USBConnection::Read(char *data, int length) {
